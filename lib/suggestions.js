  ...
  switch (goodTemp.length) {
    ...
    case 2:
      const bestDay = goodTemp.reduce((acc, curr) => {
        if (acc.cloudcover < curr.cloudcover) {
          return acc;
        } else {
          return curr;
        }
      ...
      const reason = bestDay.conditions;
      const multipleOptionDay = dateToStringName(bestDay.datetime);
      return `This weekend looks nice for a picnic, ${multipleOptionDay} is best because it’s ${reason} .`;
    default:
